      ...
      module Options
        class Fractional
          ...
          def before_union_elements
            separator = options[:fractional_separator] || translations.fraction_separator(options)
            options[:union_separator] = separator
            super
          end

          def fraction_significance
            translations.micros full_fraction.fraction_capacity, figures.reverse.join.to_i
          end
