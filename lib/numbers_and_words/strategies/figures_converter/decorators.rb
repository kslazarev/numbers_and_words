      ...
      module Decorators
        ...
        class << self
          ...
          def decorator_class method_name
            method_name ? full_class_name(method_name).constantize : Decorators::Base
          end

          def full_class_name method_name
            "#{self.name}::#{language_class_name}::#{method_name.to_s.camelcase}"
          end
